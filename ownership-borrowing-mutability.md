...
In Azoth, mutability is about more than just exclusive access, though it is still somewhat coupled. This can be clearly seen in the fact that owned references can be mutable or immutable. It is also clear that the declaration of classes and structs as mutable or immutable has nothing to do with exclusive access. Though, immutable structs are much more likely to be copy types.
